...
esigma = (esum1+esum2)/(N-K);
delta = @(x1,x2) [x1 x2]*inv(esigma)*emu1'-1/2*emu1*inv(esigma)*emu1'-[x1 x2]*inv(esigma)*emu2'+1/2*emu2*inv(esigma)*emu2';
fimplicit(delta,"g");
...
fimplicit(G,"k--");legend("sample1","sample2","LDA","Bayes(theoretical)","Bayes(estimated)",'Location','northwest')
